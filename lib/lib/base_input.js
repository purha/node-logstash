...
BaseInput.prototype.configure_unserialize = function(serializer) {
  ...
  else if (serializer == 'json_logstash_hex_support') {
    this.unserialize_data = this.unserialize_data_json_hex_support;
  }
...
BaseInput.prototype.unserialize_data_json_hex_support = function(data, ok_callback, parse_fail_callback) {
  try {
    var parsed = JSON.parse(data.replace(/\\x([0-9A-Fa-f]{2})/g, function(a,b) { 
